...
class _LoginState extends State<MyUserChangePassword> {
  ...
  void submit()async{
    ...
    String oldpassword = sh.getString('old').toString();

    if(oldpassword!=old)
      {

        Fluttertoast.showToast(msg: 'Missmatch in old passwrod. Try again after login');
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (status == 'ok') {
            Fluttertoast.showToast(msg: ' Password Updated successfully. Please login with your new password');


            Navigator.push(context, MaterialPageRoute(
              builder: (context) => LoginNewPage(),));
          } else {
            Fluttertoast.showToast(msg: 'Failed to update password. Please try again after login');
            Navigator.push(context, MaterialPageRoute(
              builder: (context) => LoginNewPage(),));
          }
        }
        else {
          Fluttertoast.showToast(msg: 'Network Error or failed to update password');
        }
      }
      catch (e) {
